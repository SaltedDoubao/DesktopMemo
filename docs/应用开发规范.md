...
│   │   │   └── CountToVisibilityConverter.cs # 数字到可见性转换器
│   │   └── Resources/              # 样式与资源
...
- `DesktopMemo.App/ViewModels/MainViewModel.cs`：应用核心逻辑，处理备忘录列表、编辑状态、页面切换与设置面板。
- `DesktopMemo.App/ViewModels/TodoListViewModel.cs`：待办事项视图模型，处理待办事项的增删改查和状态切换。
- `DesktopMemo.Infrastructure/Repositories/FileMemoRepository.cs`：文件存储实现，维护 `content` 目录和 `index.json`。
- `DesktopMemo.Infrastructure/Repositories/JsonTodoRepository.cs`：JSON 待办事项存储实现，维护 `todos.json`。
- `DesktopMemo.App/Converters/CountToVisibilityConverter.cs`：数字到可见性转换器，用于 UI 显示控制。
