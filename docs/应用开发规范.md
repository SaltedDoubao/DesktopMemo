...
- **数据存储**：默认写入可执行文件目录下的 `/.memodata`
  - 备忘录：Markdown 文件（YAML Front Matter） + `index.json` 索引
  - 待办事项：`todos.json` 文件
  - 应用设置：`settings.json` 文件
...
**前端应用层 (DesktopMemo.App)**
- `ViewModels/MainViewModel.cs`：应用核心逻辑，处理备忘录列表、编辑状态、页面切换、设置面板、主题切换等。
- `ViewModels/TodoListViewModel.cs`：待办事项视图模型，处理待办事项的增删改查和状态切换。
- `ViewModels/MemoListViewModel.cs`：备忘录列表视图模型。
- `Views/ConfirmationDialog.xaml`：通用确认对话框，支持自定义消息和"不再显示"选项。
- `Views/ExitConfirmationDialog.xaml`：退出确认对话框。
- `Converters/CountToVisibilityConverter.cs`：数字到可见性转换器，用于 UI 显示控制（如待办事项数量徽章）。
- `Localization/LocalizationService.cs`：多语言本地化服务，支持中英日韩繁体等语言。
- `Resources/Styles.xaml`：全局样式定义。
- `Resources/GlassResources.xaml`：玻璃效果（Aero）资源字典。
- `Resources/Themes/Light.xaml` 和 `Dark.xaml`：浅色和深色主题资源。

**核心领域层 (DesktopMemo.Core)**
- `Contracts/`：所有接口定义，实现依赖倒置原则。
- `Models/Memo.cs`：备忘录领域模型，包含同步状态字段。
- `Models/TodoItem.cs`：待办事项领域模型。
- `Models/WindowSettings.cs`：窗口设置模型（位置、大小、透明度等）。
- `Constants/WindowConstants.cs`：窗口相关常量（如默认透明度、尺寸等）。
- `Helpers/TransparencyHelper.cs`：透明度计算辅助类，统一管理透明度百分比转换。
- `Helpers/DebounceHelper.cs`：防抖动辅助类，用于优化频繁触发的操作（如窗口大小调整）。

**基础设施层 (DesktopMemo.Infrastructure)**
- `Repositories/FileMemoRepository.cs`：基于文件的备忘录存储实现（v2.2.0 及以前版本）。
- `Repositories/SqliteIndexedMemoRepository.cs`：基于 SQLite 的备忘录存储实现（v2.3.0+，带全文搜索索引）。
- `Repositories/JsonTodoRepository.cs`：基于 JSON 文件的待办事项存储实现。
- `Repositories/SqliteTodoRepository.cs`：基于 SQLite 的待办事项存储实现（v2.3.0+）。
- `Services/JsonSettingsService.cs`：JSON 格式的应用设置服务。
- `Services/MemoSearchService.cs`：备忘录搜索服务，支持关键词搜索。
- `Services/WindowService.cs`：窗口服务，封装 Win32 API 调用（置顶、透明度等）。
- `Services/TrayService.cs`：系统托盘服务，管理托盘图标和菜单。
- `Services/MemoMigrationService.cs`：备忘录数据迁移服务（用于版本升级）。
- `Services/TodoMigrationService.cs`：待办事项数据迁移服务（用于版本升级）。
...
- **数据问题排查**
  - 备忘录数据：检查 `/.memodata/content` 中的 Markdown 文件及 `index.json`
  - 待办事项数据：检查 `/.memodata/todos.json` 文件
  - 应用设置：检查 `/.memodata/settings.json` 文件
...
DesktopMemo 计划添加基于 MySQL 的云存储和同步功能。该功能将允许用户在多个设备之间同步他们的备忘录和待办事项。

### 当前状态
- ✅ 已实现本地 SQLite 存储（v2.3.0）
- ✅ 为数据模型添加了同步状态字段
- 📝 已定义 MySQL 集成规范
...
### 实现指南
有关详细的技术规范、API 设计、数据模型和实现指南，请参阅：
...
### 参与贡献
如果您有兴趣为 MySQL 集成做贡献：
1. 阅读 [MySQL 集成规范](MySQL-集成规范.md)
2. 查看标记为 `cloud-sync` 的现有 issue
3. 在实现之前在 issue 中讨论您的方法
4. 遵循本文档中概述的编码标准和异步编程最佳实践