...
graph TB
    ...
    subgraph "DesktopMemo.Infrastructure (基础设施层)"
        I1[Repositories 模块]
        I2[Services 模块]
    end
...
sequenceDiagram
    ...
    UI->>VM: 用户点击 "保存"
    VM->>VM: 执行 SaveMemoCommand
    VM->>Repo: UpdateAsync(memo)
    Repo->>FS: 写入 Markdown 文件
    Repo->>DB: 更新 SQLite 索引
    DB-->>Repo: 成功
    Repo-->>VM: 成功
    VM->>VM: 更新 Memos 集合
    VM-->>UI: 属性通知 (INotifyPropertyChanged)
    UI->>UI: 刷新 UI
```
